...
Connectance values ranged between `r round(min_connectance, 2)` to `r round(max_connectance, 2)` ($\bar{x}$ = `r round(mean_connectance, 2)`) and followed a negative exponential relationship with the number of species per network (Kendall $\tau$ = `r round(connectance_spp_mean_corr_tau, 2)`, _P_ < 0.01; **Figure 6a**). Nestedness values (NODFc) ranged between `r round(min_nodfc, 2)` to `r round(max_nodfc, 2)` ($\bar{x}$ = `r round(mean_nodfc, 2)`), and were independent of the mean number of species (Kendall $\tau$ = `r round(normalised_nestedness_spp_mean_corr_tau, 2)`, _P_ = `r round(normalised_nestedness_spp_mean_corr_pval, 2)`; **Figure S4**). Empirical networks did not show statistically different nestedness to the simulated ones (**Figure 6b** and **Figure S5**). The 'curveball' method for binary networks resulted in `r round(under_z_percent_curveball, 2)`% of networks statistically less nested than null expectations, `r round(no_diff_z_percent_curveball, 2)`% showing no difference, and `r round(over_z_percent_curveball, 2)`% being more nested. The 'quasiswap_count' algorithm for quantitative networks resulted in `r round(under_z_percent_quasiswap, 2)`% of networks being less nested, `r round(no_diff_z_percent_quasiswap, 2)`% showing no difference and `r round(over_z_percent_quasiswap, 2)`% being more nested. Although latitude has a statistically significant impact on residual connectance, it explains only a small portion of the observed variability in residual connectance and nestedness across networks. (connectance: R$^2$ = `r round(connectance_lat_r2, 2)`, _P_ = `r round(connectance_lat_pval, 2)`, **Figure 6c**; NODFc: R$^2$ = `r round(normalised_nestedness_lat_r2, 2)`, _P_ = `r round(normalised_nestedness_lat_pval, 2)`, **Figure 6d**). Overall, networks towards higher latitudes showed lower residual connectance and higher nestedness than networks located at lower latitudes. Note that residual connectance and normalised nestedness showed a moderate negative correlation (Kendall $\tau$ = `r round(resid_connectance_nestedness_corr_tau, 2)`, _P_ = `r round(resid_connectance_nestedness_corr_pval, 2)`).
