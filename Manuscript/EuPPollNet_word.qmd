...
Connectance and nestedness did not change across the latitudinal range of studies (connectance: R$^2$ = `r round(connectance_lat_r2, 2)`, _P_ = `r round(connectance_lat_pval, 2)`, **Figure 6a**; NODFc: R$^2$ `r round(normalised_nestedness_lat_r2, 2)`, _P_ = `r round(normalised_nestedness_lat_pval, 2)`, **Figure 6b**). However, differences were found for connectance but not nestedness across bioclimatic regions. Specifically, the alpine region differed statistically in connectance to the other bioclimatic regions (Tukey test _P_ < 0.01). In addition, the atlantic and continental bioclimatic regions also showed statistical meaningful differences in connectance (Tukey test _P_ < 0.01). Connectance values ranged between `r round(min_connectance, 2)` to `r round(max_connectance, 2)` ($\bar{x}$ = `r round(mean_connectance, 2)`) and followed an negative exponential relationship with the geometric mean of plant and pollinator species per network (Kendall $\tau$ = `r round(connectance_spp_mean_corr_tau, 2)`, _P_ < 0.01; **Figure 6c**). Nestedness values (NODFc) ranged between `r round(min_nodfc, 2)` to `r round(max_nodfc, 2)` ($\bar{x}$ = `r round(mean_nodfc, 2)`), and were independent of the mean number of species (Kendall $\tau$ = `r round(normalised_nestedness_spp_mean_corr_tau, 2)`, _P_ = `r round(normalised_nestedness_spp_mean_corr_pval, 2)`; **Figure S3**). Finally, we found that `r round(over_z_percent, 2)`% of networks were statistically more nested than expected by chance, `r round(no_diff_z_percent, 2)`% showed no statistical difference, and `r round(under_z_percent, 2)`% were less nested than null expectations (**Figure 6d**).
...
Major changes in NW Europe occurred pre-1990
https://doi.org/10.1111/ele.12121

EuPPollNet represents the largest database of plant-pollinator interactions compiled to date and is a work in progress database as there are more studies, unsampled habitats, species, etc...
...
theme(plot.title = element_text(size = 14, face = "bold"),
      ...
      axis.text = element_text(face = "bold", size = 1))
