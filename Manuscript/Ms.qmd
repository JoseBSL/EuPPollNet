...
The different sites per study were classified with an habitat by the authors. As these habitats are not standardize across studies, we homogenized the habitat classification with the help of land cover information and visual checks on current satellite imagery. The land cover information was extracted from Corine Land Cover (CLC, version 2018) with the help of the Terra package [@hijmans2022]. Then, we checked the coordinate sites on Google Earth to double check that the categories that we are assigning after evaluating the author's habitat classification and the land cover classification are meaningful. However, this is a non fully objective process and we advise authors to revise this classification if they intend to rely on this field for their analyses. Moreover, we assigned each study site to a biogeographical region, which are areas with similar environmental conditions that harbor similar groups of species. We believe this could be a useful field for organizing the different studies given the different environmental conditions and pool of species. The biogeographical regions were downloaded from the European Environment Agency (version 2016) and match with the different study site coordinates.


Phylo for plants and pollinators!

```{r echo=FALSE, message=FALSE, cache=FALSE, warning=FALSE, out.width="95%", fig.cap= "\\textbf{Figure 2}. Proportion of the major pollinator orders by habitat types and bioclimatic regions in the SafeNet database. The orders, from left to right, include Hymenoptera, Diptera, Lepidoptera and Coleoptera. The horizontal barplot on the right indicates the number of studies that were conducted on each habitat type or bioclimatic region. Note that a single study can countribute to more than one habitat or bioclimatic region. Areas with a greater number of studies are more likely to depict accurate proportions of pollinators in those systems.", fig.width=3.8, fig.height=3.8}
