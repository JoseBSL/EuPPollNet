...
This European metaweb consist of datasets of plant-pollinator interactions compiled initially by a wide number of researchers and institutions within the European continent. This dataset consist of `r study_number` independent published and unpublished studies conducted during the time period `r min_year` - `r max_year` on `r country_number` different countries (**Figure 1a**). The dataset contains a total of `r prettyNum(interaction_number, big.mark=",")` distinct interactions, considering interaction as the contact of a given pollinator to the reproductive structure of a particular plant, from `r prettyNum(pollinator_spp_number, big.mark=",")` pollinator and `r prettyNum(plant_spp_number, big.mark=",")` plant species. These different studies differ in sampling effort and methodology, although most studies took place within a single flowering season (PERCENTAGE) with an average of XXX sampling days (range XXX to XXXX) over a time span of XXXX days, and documented interactions mostly by using `r  most_common_method[[1]]` (`r  most_common_method[[3]]`%). 

Explain % of taxonomic ranks at order level that constitute the included pollinators. Something with the different pollinators is going to be graph 2.
